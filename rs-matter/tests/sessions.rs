...
fn test_session_table_capped_and_recycled() {
    ...
    let over_cap = block_on(Exchange::initiate_plaintext(
        &matter,
        &crypto,
        Address::new(),
    ));
    assert!(
        matches!(
            over_cap.as_ref().map_err(|e| e.code()),
            Err(ErrorCode::NoSpaceSessions)
        ),
        "expected NoSpaceSessions once the table is full and busy, got {over_cap:?}"
    );

    // Release the busy exchanges. The sessions remain in the table but are now
    // idle and therefore evictable.
    drop(held);

    // Further allocations succeed by evicting the LRU idle session each time —
    // proof the table recycles at the cap rather than growing past it.
...
fn test_exchange_slots_exhausted_and_released() {
    ...
    assert!(
        ...
        "expected NoSpaceExchanges once all exchange slots are taken, got {over_cap:?}"
    );
