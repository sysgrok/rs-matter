...
//! - Backed by a slice instead of by an owned array
//! - In-place initialization of the vec itself with `SliceVec::init() -> impl Init<Self>`
//! - In-place initialization of the vec members with `SliceVec::push_init(init: I) -> Result<(), ()>`
...
/// let mut buf = [_, 9];
/// let mut vec = SliceVec::new(&mut buf);
/// vec.push(1);
/// vec.push(2);
///
/// assert_eq!(vec.len(), 2);
...
impl<'a, T> SliceVec<'a, T> {
    /// Constructs a new, empty vector using the supplied slice as a backing store
    ///
    /// # Examples
    ///
    /// ```
    ...
    /// let mut x = SliceVec::new(&mut buf);
    /// ```
    /// `SliceVec` `const` constructor; wrap the returned value in [`SliceVec`].
    ...
    /// Appends an item with the provided item initializer - `init`
    /// to the back of the collection
    ///
    /// Returns an error generated by `f` if the vector is full
...
impl<T> fmt::Debug for SliceVec<'_, T>
where
    T: fmt::Debug,
{
...
#[cfg(feature = "defmt")]
impl<T> defmt::Format for SliceVec<'_, T>
where
    T: defmt::Format,
{
...
impl<'a, T> Extend<&'a T> for SliceVec<'_, T>
where
    T: 'a + Copy,
{
    fn extend<I>(&mut self, iter: I)
...
impl<'a, T> IntoIterator for &'a SliceVec<'_, T> {
    type Item = &'a T;
...
impl<'a, T> IntoIterator for &'a mut SliceVec<'_, T> {
    type Item = &'a mut T;
...
// impl<T> FromIterator<T> for SliceVec<'_, T> {
...
// /// An iterator that moves out of an [`SliceVec`][`SliceVec`].
// ///
// /// This struct is created by calling the `into_iter` method on [`SliceVec`][`SliceVec`].
...
impl<A, B> PartialEq<SliceVec<'_, B>> for SliceVec<'_, A>
where
    A: PartialEq<B>,
{
    fn eq(&self, other: &SliceVec<'_, B>) -> bool {
        <[A]>::eq(self, &**other)
    }
}

// SliceVec<'_, A> == [B]
impl<A, B> PartialEq<[B]> for SliceVec<'_, A>
where
    A: PartialEq<B>,
{
...
// [B] == SliceVec<'_, A>
impl<A, B> PartialEq<SliceVec<'_, A>> for [B]
where
    A: PartialEq<B>,
{
    fn eq(&self, other: &SliceVec<A, N>) -> bool {
        <[A]>::eq(other, &self[..])
    }
}
...
// &[B] == SliceVec<'_, A>
impl<A, B, const N: usize> PartialEq<SliceVec<'_, A>> for &[B]
where
    A: PartialEq<B>,
{
    fn eq(&self, other: &SliceVec<'_, A>) -> bool {
        <[A]>::eq(other, &self[..])
    }
}

// SliceVec<'_, A> == &mut [B]
impl<A, B, const N: usize> PartialEq<&mut [B]> for SliceVec<'_, A>
where
    A: PartialEq<B>,
{
...
// &mut [B] == SliceVec<'_, A>
impl<A, B, const N: usize> PartialEq<SliceVec<'_, A>> for &mut [B]
where
    A: PartialEq<B>,
{
...
impl<A, B, const M: usize> PartialEq<[B; M]> for SliceVec<'_, A>
where
    A: PartialEq<B>,
{
...
// [B; M] == SliceVec<'_, A>
// Equality does not require equal capacity
impl<A, B, const M: usize> PartialEq<SliceVec<'_, A>> for [B; M]
where
    A: PartialEq<B>,
{
    fn eq(&self, other: &SliceVec<'_, A>) -> bool {
...
impl<T> Eq for SliceVec<'_, T> where T: Eq {}

impl<T> PartialOrd<SliceVec<'_, T>> for SliceVec<'_, T>
where
    T: PartialOrd,
{
    fn partial_cmp(&self, other: &SliceVec<'_, T>) -> Option<Ordering> {
        PartialOrd::partial_cmp(&**self, &**other)
    }
}

impl<T> Ord for SliceVec<'_, T>
where
    T: Ord,
{
    #[inline]
...
impl<T> ops::Deref for SliceVec<'_, T> {
    type Target = [T];
...
impl<T> ops::DerefMut for SliceVec<'_, T> {
    fn deref_mut(&mut self) -> &mut [T] {
        self.as_mut_slice()
    }
}

impl<T> AsRef<SliceVec<'_, T>> for SliceVec<'_, T> {
    #[inline]
    fn as_ref(&self) -> &Self {
        self
    }
...
impl<T> AsMut<SliceVec<'_, T>> for SliceVec<'_, T> {
    #[inline]
...
mod tests {
    ...
    macro_rules! buf {
        [$type: ty; $len: expr] => {
            [core::mem::MaybeUninit::<$type>::uninit(); $len]
        }
    }
    ...
    #[test]
    fn eq() {
        let mut xbuf = buf![i32; 4];
        let mut ybuf = buf![i32; 8];
        let mut xs = SliceVec::new(&mut xbuf);
        let mut ys = SliceVec::new(&mut ybuf);
    ...
    fn cmp_heterogenous_size() {
        ...
        let mut ybuf = buf![i32; 8];
        let mut xs = SliceVec::new(&mut xbuf);
        let mut ys = SliceVec::new(&mut ybus);
    ...
    fn cmp_with_arrays_and_slices() {
        let mut buf = buf![i32; 12];
        let mut xs = SliceVec::new(&mut buf);
        xs.push(1).unwrap();
