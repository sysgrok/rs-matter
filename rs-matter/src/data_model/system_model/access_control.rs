...
use crate::acl::{self, AclEntry, AclEntryWithFabric};
                        ...
                        for fabric in fabric_mgr.iter() {
                            if !attr.fab_filter || fabric.fab_idx().get() == attr.fab_idx {
                                for entry in fabric.acl_iter() {
                                    AclEntryWithFabric::new(entry, Some(fabric.fab_idx()))
        ...
        match op {
            ListOperation::AddItem | ListOperation::EditItem(_) => {
                if let ListOperation::EditItem(index) = op {
                    fabric_mgr.acl_update_tlv(fab_idx, *index as _, data)?;
                } else {
                    fabric_mgr.acl_add_init(fab_idx, AclEntry::init_from_tlv(data.clone()))?;
                }
...
mod tests {
    ...
    fn acl_cluster_add() {
        ...
        // Test a simple add to fabric 1

        acl.write_acl_attr(&mut fab_mgr, &ListOperation::AddItem, &data, FAB_1)
            .unwrap();

        let verifier = AclEntry::new(Privilege::VIEW, AuthMode::Case);
        let fabric = fab_mgr.get(FAB_1).unwrap();
        assert_eq!(fabric.acl_iter().count(), 1);
    ...
    fn acl_cluster_edit() {
        ...
        let mut verifier = [
            (FAB_2, AclEntry::new(Privilege::VIEW, AuthMode::Case)),
            (FAB_1, AclEntry::new(Privilege::VIEW, AuthMode::Case)),
            (FAB_2, AclEntry::new(Privilege::ADMIN, AuthMode::Case)),
        ];

        for (fab_idx, i) in &verifier {
            fab_mgr.acl_add(*fab_idx, i.clone()).unwrap();
        }
        let acl = AccessControlCluster::new(Dataver::new(0));
        ...
        acl.write_acl_attr(&mut fab_mgr, &ListOperation::EditItem(1), &data, FAB_2)
            .unwrap();
        verifier[2].1 = new;
